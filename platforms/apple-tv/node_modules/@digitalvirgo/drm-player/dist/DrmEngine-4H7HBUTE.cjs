...
// src/player/engines/DrmEngine.ts
var installed = false;
var clppModule = null;
async function ensureInstalled() {
  if (installed) return;
  try {
    clppModule = await Promise.resolve().then(() => _interopRequireWildcard(require("./cl.core-XGKDEPHN.cjs")));
    await Promise.resolve().then(() => _interopRequireWildcard(require("./cl.mse-VYY3FMU6.cjs")));
    await Promise.resolve().then(() => _interopRequireWildcard(require("./cl.dash-BV34FZ7L.cjs")));
    await Promise.resolve().then(() => _interopRequireWildcard(require("./cl.hls-KTKWF4AP.cjs")));
    await Promise.resolve().then(() => _interopRequireWildcard(require("./cl.crypto-ARKXTTZL.cjs")));
    await Promise.resolve().then(() => _interopRequireWildcard(require("./cl.onboard-QPF5MSG4.cjs")));
...
var DrmEngine = class {
  ...
  async load(request) {
    ...
    try {
      ...
      const config = {
        ...
        muted: _nullishCoalesce(request.muted, () => ( false)),
        drm: {
          env: cfg.drmEnv,
          customData: {
  ...
  play() {
    _optionalChain([this, 'access', _ => _.player, 'optionalAccess', _2 => _2.play, 'call', _3 => _3()]);
  }
  pause() {
    _optionalChain([this, 'access', _4 => _4.player, 'optionalAccess', _5 => _5.pause, 'call', _6 => _6()]);
  }
  seek(seconds) {
    if (this.player) {
      const duration = _optionalChain([this, 'access', _7 => _7.player, 'access', _8 => _8.getDuration, 'optionalCall', _9 => _9()]) || 0;
  ...
  bindPlayerEvents(clpp) {
    if (!this.player) return;
    this.player.on(clpp.events.STATE_CHANGED, (e) => {
    ...
    this.player.on(clpp.events.ERROR, (e) => {
      if (this.isIgnorableError(e.detail)) return;
      const message = this.mapError(e.detail);
      this.updateState({ error: message, loading: false, playing: false });
    });
    this.player.on(clpp.events.LOADEDMETADATA, () => {
      const duration = _optionalChain([this, 'access', _10 => _10.player, 'optionalAccess', _11 => _11.getDuration, 'optionalCall', _12 => _12()]) || 0;
      this.updateState({ duration, loading: false });
    });
    this.timeInterval = setInterval(() => {
      if (this.player && this.state.playing) {
        const currentTime = _optionalChain([this, 'access', _13 => _13.player, 'access', _14 => _14.getPosition, 'optionalCall', _15 => _15()]) || 0;
  ...
  isIgnorableError(err) {
    const msg = String(_nullishCoalesce(_nullishCoalesce(_optionalChain([err, 'optionalAccess', _16 => _16.message]), () => ( err)), () => ( "")));
  ...
  mapError(err) {
    ...
    const code = _nullishCoalesce(err.code, () => ( err.category));
    const msg = _nullishCoalesce(_nullishCoalesce(err.message, () => ( err.detail)), () => ( String(err)));
